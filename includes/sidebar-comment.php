...
<?php
	...
	$output = $pre_HTML;
	foreach ($comments as $comment) {$output .= "<li>".get_avatar(get_comment_author_email(), 72, '', get_comment_author())."<p class='comment-author'>".strip_tags($comment->comment_author)."</p>" . "<p class='comment-content'><a href=\"" . get_comment_link($comment->comment_ID) . "\" title=\"查看 " .$comment->post_title . "\">" . strip_tags($comment->com_excerpt)."</a></p></li>\n";}
	$output .= $post_HTML;
